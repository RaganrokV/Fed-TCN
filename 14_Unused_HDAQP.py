...
#%%
class HDAQP(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, kernel_size, pool_size, dropout_rate):
        super(HDAQP, self).__init__()
        ...
        self.lstm1 = nn.LSTM(input_size=128, hidden_size=hidden_dim,
        ...
        self.lstm2 = nn.LSTM(input_size=hidden_dim,
                             ...
                             num_layers=num_layers, batch_first=True,
                             dropout=dropout_rate)
        self.fc1 = nn.Linear(hidden_dim, 168)
        self.fc2 = nn.Linear(32, 32)
        self.fc3 = nn.Linear(32, 168)
        self.dropout = nn.Dropout(dropout_rate)

    def forward(self, x):
        x = self.conv1(x.permute(0, 2, 1))  # 将输入的通道数调整为168
        x = self.relu(x)
        x = self.pool(x)
        x = self.conv2(x)
        x = self.relu(x)
...
for idx in [3]:#[3,4,5,6,7,8,9,10]
    ...
    HDAQP_net = HDAQP(input_dim, hidden_dim, num_layers, kernel_size, pool_size, dropout_rate).to(device)
    # print(fed_lstm)

    optimizer = torch.optim.Adam(HDAQP_net.parameters(), lr=0.01)
    ...
    #           train
    train_loss_all = []
    HDAQP_net.train()  # Turn on the train mode
    total_loss = 0.
    ...
    for epoch in range(200):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(HDAQP_net.parameters(), 0.1)  # 梯度裁剪，放backward和step直接，小模型可以不考虑用于缓解梯度爆炸
            optimizer.step()
        ...
        if train_loss < best_val_loss:
            ...
            best_model =HDAQP_net


        scheduler.step()

    best_model = HDAQP_net.eval()  # 转换成测试模式
    pred = HDAQP_net(testX.float().to(device))
